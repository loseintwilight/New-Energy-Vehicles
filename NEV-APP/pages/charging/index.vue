...
export default {
  ...
  methods: {
    async loadRecords(isLoadMore = false) {
      ...
      this.loading = true
      try {
        const res = await getChargingRecords({
    ...
    formatRecord(item) {
      return {
        id: item.orderId || item.id || item.recordId || item.chargingId,
        stationName: item.stationName || item.name || '',
        status: item.orderStatus === '1' || item.orderStatus === 1 ? 'completed' : 'ongoing',
        startTime: item.startTime || item.createTime || '',
        endTime: item.endTime || '',
        duration: item.durationText || item.duration || '--',
        power: item.totalEnergy || item.power || item.electricity || 0,
        cost: item.totalAmount || item.cost || item.amount || 0,
        address: item.address || item.stationAddress || '',
