  ...
  <view class="page" :class="{ 'page-ready': isReady }">
    ...
    <scroll-view scroll-y class="main-scroll" :show-scrollbar="false">
      ...
      <view class="pile-list">
        ...
        >
          <!-- 左侧彩色状态条 -->
          <view class="pc-bar" :class="'bar-status-' + item.status"></view>
          <!-- 颜色蔓延层 -->
          <view class="pc-spread" :class="'spread-status-' + item.status"></view>
          <!-- 卡片主体 -->
          <view class="pc-body">
            <!-- 顶部：编码 + 类型 + 状态 -->
            <view class="pc-top">
              <view class="pc-left">
                <text class="pc-code">{{ item.code }}</text>
                <text class="type-badge tb-dc" v-if="item.type === 'dc'">DC快充</text>
                <text class="type-badge tb-ac" v-else>AC慢充</text>
              </view>
              <view class="status-badge" :class="'sb-' + item.status">
                <view class="sb-dot" :class="{ 'dot-blink': item.status === '1' }"></view>
                <text class="sb-text">{{ item.statusText }}</text>
              </view>
            </view>
            ...
            <view class="pc-power-row">
              <view class="power-block">
                <text class="power-num" :class="item.type === 'dc' ? 'power-dc' : 'power-ac'">{{ item.power }}</text>
                <text class="power-unit">kW</text>
              </view>
              <view class="connector-tag">
                <text class="conn-text">{{ item.connector }}</text>
              </view>
            </view>

            <!-- 充电中实时数据 -->
            <view v-if="item.status === '1'" class="live-data">
              ...
              <view class="live-cell">
                ...
                <text class="live-val">{{ item.currentUser || '--' }}</text>
              </view>
            </view>
            ...
            <view class="pc-bottom">
              <view class="bottom-meta">
                <text class="bm-lbl">今日充电量</text>
                <text class="bm-val">{{ item.todayEnergy }}kWh</text>
              </view>
              <view class="bottom-meta">
                <text class="bm-lbl">今日次数</text>
                <text class="bm-val">{{ item.todayCount }}次</text>
              </view>
...
export default {
  data: function() {
    return {
      ...
      mockPiles: [
        { pileId: 1, code: 'AT-DC-01', type: 'dc', typeLabel: 'DC快充', power: 120, connector: 'GB/T', status: '0', statusText: '空闲', todayEnergy: 158.5, todayCount: 12, stationName: '济南奥体中心' },
        { pileId: 2, code: 'AT-DC-02', type: 'dc', typeLabel: 'DC快充', power: 120, connector: 'GB/T', status: '0', statusText: '空闲', todayEnergy: 142.0, todayCount: 10 },
        { pileId: 3, code: 'AT-DC-03', type: 'dc', typeLabel: 'DC快充', power: 180, connector: 'GB/T', status: '1', statusText: '充电中', todayEnergy: 128.0, todayCount: 8, currentOrderNo: 'CO20260531000001', currentUser: '张**' },
        { pileId: 4, code: 'AT-DC-04', type: 'dc', typeLabel: 'DC快充', power: 180, connector: 'CCS', status: '0', statusText: '空闲', todayEnergy: 105.0, todayCount: 7 },
        { pileId: 5, code: 'AT-AC-01', type: 'ac', typeLabel: 'AC慢充', power: 7, connector: 'Type2', status: '0', statusText: '空闲', todayEnergy: 32.0, todayCount: 18 },
        { pileId: 6, code: 'AT-AC-02', type: 'ac', typeLabel: 'AC慢充', power: 7, connector: 'Type2', status: '0', statusText: '空闲', todayEnergy: 28.0, todayCount: 15 },
        { pileId: 7, code: 'AT-AC-03', type: 'ac', typeLabel: 'AC慢充', power: 7, connector: 'Type2', status: '0', statusText: '空闲', todayEnergy: 25.0, todayCount: 14 },
        { pileId: 8, code: 'AT-AC-04', type: 'ac', typeLabel: 'AC慢充', power: 7, connector: 'Type2', status: '0', statusText: '空闲', todayEnergy: 22.0, todayCount: 12 }
      ]
    }
  },
  computed: {
    filteredPiles: function() {
      var self = this
      var list = self.mockPiles
      if (self.filterType) {
        list = list.filter(function(p) { return p.type === self.filterType })
      }
      if (self.filterStatus !== '' && self.filterStatus !== null) {
        list = list.filter(function(p) { return p.status === self.filterStatus })
      }
      if (self.searchKey) {
        var key = self.searchKey.toLowerCase()
        list = list.filter(function(p) { return p.code.toLowerCase().indexOf(key) !== -1 })
      }
      return list
    }
  },
  ...
  methods: {
    ...
    calcStats: function() {
      this.totalCount = this.mockPiles.length
      this.availableCount = 0
      this.chargingCount = 0
      for (var i = 0; i < this.mockPiles.length; i++) {
        if (this.mockPiles[i].status === '0') {
          this.availableCount++
        } else if (this.mockPiles[i].status === '1') {
          this.chargingCount++
        }
      }
    },
    ...
    onStatusChange: function(val) { this.filterStatus = val },
    ...
    doDeletePile: function(pileId, code) {
      ...
      uni.showModal({
        ...
        success: function(res) {
          if (res.confirm) {
            self.mockPiles = self.mockPiles.filter(function(p) { return p.pileId !== pileId })
...
.empty-icon-wrap {
  ...
  height: 160rpx;
  border-radius: 50%;
  background: linear-gradient(135deg, rgba(245,158,11,0.08), rgba(249,115,22,0.14));
  display: flex;
  align-items: center;
  justify-content: center;
  margin-bottom: 32rpx;
  animation: emptyFloat 3.5s ease-in-out infinite;
  box-shadow: 0 8rpx 32rpx rgba(245, 158, 11, 0.08);
}
...
.card-hover {
  ...
  box-shadow: 0 2rpx 10rpx rgba(245, 158, 11, 0.08), 0 1rpx 3rpx rgba(0, 0, 0, 0.04);
}
...
.tb-dc {
  ...
  box-shadow: 0 2rpx 8rpx rgba(217, 119, 6, 0.25);
}
...
.power-dc {
  color: #2563eb;
  text-shadow: 0 2rpx 8rpx rgba(37, 99, 235, 0.12);
}

.power-ac {
  color: #16a34a;
  text-shadow: 0 2rpx 8rpx rgba(22, 163, 74, 0.12);
}
...
.live-data {
  ...
  flex-direction: row;
  align-items: center;
  background: linear-gradient(135deg, rgba(37, 99, 235, 0.05), rgba(59, 130, 246, 0.10));
  border-radius: 14rpx;
  padding: 16rpx 18rpx;
  margin-bottom: 16rpx;
...
.pc-bottom {
  ...
  flex-direction: row;
  align-items: center;
  border-top: 1rpx solid rgba(245, 158, 11, 0.07);
  padding-top: 14rpx;
  margin-top: 4rpx;
}
